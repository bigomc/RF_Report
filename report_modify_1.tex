...
In the diagram it is shown that the robot contains five different reference frames which are described by the parameters in table \ref{tab:forward.dh}. Then, the homogeneous transformation between frames can be described by a set of parameters in the table, the transformation outcome is
...
\begin{figure}
\begin{center}
\includegraphics[width=0.8\textwidth]{image/diagram}
\caption{Lynxmotion robot diagram}
\label{fig:forward.diagram}
\end{center}
\end{figure}

Every matrix represents the orientation and translation of every frame with respect to the previous reference frame. In order to derive the forward kinematics of the robots it is necessary to culculate the orientation and translation of the end effector which is the frame at the point $O_5$. The transformation expression of the end effector should be regarding to the world frame whose origin is at point $O_0$.

In order to get the translational and rotated parametes of the end effector with respect to frame 0, the equation of matrix multiplication below can be used to compute for $\T{0}{5}$

\begin{equation}
\T{0}{5} = \T{0}{1} \T{1}{2}\T{2}{3}\T{3}{4}\T{4}{5}
\end{equation}

After performing the matrix multiplication, the outcome will be the homogeneous matrix that represents the orientation and translation of the frame attached at the end effector, and it has the following form:
...
where $R$ is the rotation matrix of the fifth frame with respect to main frame and ${}^0O_5$ is the coordinate vector of point $O_5$ with respect to main frame. This point is also the desired one of the end effector. This matrix is a function of the variables $\theta_1 ... \theta_5$.
...
To compute the forward kinematics of the Lynxmotion, we need to created a function in Matlab whose input is the vector $Q=[\theta_1 \theta_2 \theta_3 \theta_4 \theta_5]^T$ and this function outputs the position ${}^0P$ and orientation $R$ of the end effector automatically. The code of this function can be found in appendix \ref{apendix:LynxFK}.

\subsection{Workspace}
As for the workspace, it is essential to measure the physical operational range of every joint variables $\theta_i$, and the results are shown in table \ref{tab:workspace.range}.
...
Once the range of the variables has been determined, the movement of every joint can be simulated in its corresponding operational range, thus we can create the reachable 3D space of the end effector. This space generated by all the points that can be reached is called workspace. Relevant code of performing the simulation can be found in appendix \ref{apendix:workspace}.

\begin{figure}
\begin{center}
\includegraphics[width=\textwidth]{../Part1/Workspace}
\caption{Lynxmotion robot workspace}
\label{fig:workpace.workspace}
\end{center}
\end{figure}

The workspace of the Lynxmotion robot is depicted in figure \ref{fig:workpace.workspace}, the left top picture shows a 3D view of the space, while the other three views show the planes YX, ZX and ZY respectively.
...
Since the x and y position of the end effector are only decided by the rotation of joint 1, we can project the manipulator into the horizontal plane of the frame 0. Therefore,the expression of  $\theta_1$ can be derived by
...
\begin{figure}
\begin{center}
\includegraphics[width=\textwidth]{image/theta2,theta3}
...
\begin{figure}
\begin{center}
\includegraphics[width=\textwidth]{../Part1/Points}
...
\includegraphics[scale=1]{../Part1/JointTrayectories}
...
\verbatiminput{../Part1/LynxFK.m}

\subsection{Workspace (workspace script)}
\label{apendix:workspace}
\verbatiminput{../Part1/workspace_script.m}
