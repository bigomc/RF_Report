...
The diagram shows that the robot contains five different reference frames which are described by the parameters in table \ref{tab:forward.dh}. Correspondingly, the homogeneous transformation between frames can be described as below:
...
Every matrix represents the orientation and displacement of each frame regarding to the previous frame. In order to derive the forward kinematics of the robots it is necessary to know the orientation and position of the end effector, the frame at the point $O_5$, with respect to the world frame whose origin is at point $O_0$.
...
To compute the forward kinematics in Matlab, a function is created to receive the vector $Q=[\theta_1 \theta_2 \theta_3 \theta_4 \theta_5]^T$ as input and output the position ${}^0P$ and orientation $R$ of the end effector, which is depicted in figure \ref{fig:forward.block_diagram}. The codes of the function are in appendix \ref{apendix:LynxFK}.
...
The operational range of every joint variales $\theta_i$ is necessary for tracing out the workspace of the robot.  The relevant parameters are in table \ref{tab:workspace.range}.
...
Once the range of the variables has been determined, the movement of every joint can be simulated to create a reachable 3D space for the end effector. The related simulation code can be found in appendix \ref{apendix:lynx_workspace}.
...
Figure \ref{fig:workpace.workspace} presents the workspace of the Lynxmotion robot. Specifically, the top left picture shows a 3D view of the space, while the others intuitively present a 2D view in the planes of YX, ZX and ZY respectively.
...
Compared with forward kinematics, inverse kinematics works in an oppisite way. It calcultes the set of joint values ($\theta_i^{\prime}$) through loading the pair data $P$ and $R$ which are stored in the previous files. Then, in order to validate the results of the inverse kinematics process, the robot will compute the forward kinematics using the given output which should produce a new set of points, called $P^{\prime}$ and $R^{\prime}$, which should be very similar to those used as input for inverse kinematics. This process is depicted by figure \ref{fig:inverse_test.diagram}.
...
To provide a quantitative measure of the result, an error function is defined as $e_i$ to compute the difference between the desired position ($P$) and the obtained position ($P_i^{\prime}$) at the point $i$.  The results of the first solution are shown in table \ref{tab:inverse_test.error}.
...
The approach used to implement the desired trajectories for the robot was to use via point to create the paths to be followed and therefore compute the coefficients of the polynomials that best fit to those paths.

The considered polynomial trajectory was 4-3-4. This means that the first and last trajectory is a polynomial of fourth degree whilst for the inner trajectories a polynomial of third degree is used.

\begin{equation}
q(t) = 
  \begin{cases} 
  q_1(t)= a_5t^5 + a_4t^4 + a_3t^3 + a_2t^2 + a_1t + a_0 & : t_0 \leq t < t_1 \\
   q_2(t)= b_3t^3 + b_2t^2 + b_1t + b_0 & : t_1 \leq t < t_2 \\
   q_3(t)= c_5t^5 + c_4t^4 + c_3t^3 + c_2t^2 + c_1t + c_0 & : t_2 \leq t < t_f
  \end{cases}
\end{equation}

Finally, to provide the trajectory its only necessary to compute the values of the coefficients to fully describe the piecewise trajectory at the desired points.
...
\section{Part 2}
\subsection{Inverse Kinematics}
The manipilator of this part is a planar parallel robot which applied in surgery. The schematic diagram is shown in  figure \ref{fig:Parallel}. Concretely, The task os this section is to find the solutions for $\psi_i$ and $\theta_i$ given the position coordiante of $C$ with respect to B and the rotation angle $a$ of the platform.
\begin{figure}[htbp] 
\begin{center}
...
\subsection{Workspace}
Considering that there are three legs in this manipulators, applying geometirc method to work out the workspace might be complexe in some sense. Therefore, algebraic approach relying on the powerful computation of Matlab is also a feasible way to calculate the workspace of the end effector. The core thought is that the real root of the equation\ref{eq:parallel_t} exist for each leg when the point is reachable for the manipulator. Namely, three pairs of $t_1 and t_2$ for every leg are real number within the workspace. Correspondingly, we apply the if statement to judge the value of $t_1 and t_2$. This is a robust method to fit the real workspace by thousands of points forming a region.Then, through specify the coordinate range of point $C$, we can derive the workspace. Concretely, we select a wide range for $X_c,Y_c$ and draw the workspace for different $a$ respectively.
...
It is obvious from the picture that different angle $a$ correspond to various shapes of workspace. Furthermore, we find that when $a=0^{\circ}$, the workspace has the biggest shape. It is reasonable as the base plate and platform are parallel when $a=0^{\circ}$, which enable three legs to stretch themselves to the most. The comparison of different shapes of workspace under diverse angle $a$ is shown in figure \ref{fig:parallel_workspace2}
